...
namespace Eigen {
  extern DFTI_DESCRIPTOR_HANDLE *pfft_handle;
  extern DFTI_DESCRIPTOR_HANDLE *pifft_handle;
...
public:
  ...
  ~Timer() {
    ...
    std::cout << name << ": " << ((end.tv_sec - start.tv_sec) * 1000 + (end.tv_usec - start.tv_usec) / 1000) << " ms" << std::endl;
  }

  void dumpTime(const char *str) {
    gettimeofday(&end, NULL);
    std::cout << str << ": " << ((end.tv_sec - start.tv_sec) * 1000 + (end.tv_usec - start.tv_usec) / 1000) << " ms" << std::endl;
  }
 ...
 private:
  ...
  void fft_init(int size) {
    if (pfft_handle == NULL) {
      ...
      pfft_handle = new DFTI_DESCRIPTOR_HANDLE[num_threads];

      for (int i = 0; i < num_threads; ++i) {
        status = DftiCreateDescriptor(&pfft_handle[i], DFTI_SINGLE, DFTI_COMPLEX, 1, size);
        status = DftiCommitDescriptor(pfft_handle[i]);
      }
    }
  }

  void ifft_init(int size) {
    if (pifft_handle == NULL) {
      MKL_LONG status;
      int num_threads = omp_get_max_threads();
      pifft_handle = new DFTI_DESCRIPTOR_HANDLE[num_threads];

      for (int i = 0; i < num_threads; ++i) {
        //Specify size and precision
        status = DftiCreateDescriptor(&pifft_handle[i], DFTI_SINGLE, DFTI_COMPLEX, 1, size);
        //Scale down the output
        status = DftiSetValue(pifft_handle[i], DFTI_BACKWARD_SCALE, 1.0f / size); 
        //Finalize the descriptor
        status = DftiCommitDescriptor(pifft_handle[i]); 
      }
    }
  }

  void fft(std::complex<float> *data, int tid){
    DftiComputeForward(pfft_handle[tid], data);
    //Free the descriptor
    //status = DftiFreeDescriptor(&descriptor); 
  }

  void ifft(std::complex<float> *data, int tid) {
    DftiComputeBackward(pifft_handle[tid], data); 
  }
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void evalToBuf(OutputScalar* data) {
    ...
    for (size_t i = 0; i < m_fft.size(); ++i) {
      ...
      if (FFTDir == FFT_FORWARD) {
        fft_init((int)line_len);
      } else {
        ifft_init((int)line_len);
      }
      ...
      for (Index partial_index = 0; partial_index < m_size / line_len; ++partial_index) {
        ...
        for (int j = 0; j < line_len; ++j, offset += stride) {
          buf[offset] = MakeComplex<internal::is_same<InputScalar, RealScalar>::value>()(m_impl.coeff(offset));
        }

        if (FFTDir == FFT_FORWARD) {
          fft((std::complex<float> *)&buf[base_offset], tid);
        } else {
          ifft((std::complex<float> *)&buf[base_offset], tid);
        }

        if (!write_to_out) {
          for (Index i = base_offset; i < base_offset + line_len; ++i) {
